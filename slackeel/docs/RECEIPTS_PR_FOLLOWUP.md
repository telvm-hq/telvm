# Receipts PR — follow-up comment for GitHub

Paste the block below as a **second comment** on the Receipts PR (after the main description) so reviewers can approve from mobile with full context in-thread.
...
If you use [GitHub CLI](https://cli.github.com/) and the PR is already open, post the body-only file from the repo root:

```bash
gh pr comment <PR_NUMBER> --body-file slackeel/docs/receipts_pr_followup_github.body.md
```

Body-only copy lives in [receipts_pr_followup_github.body.md](receipts_pr_followup_github.body.md) so the PR comment does not include this README-style header.
