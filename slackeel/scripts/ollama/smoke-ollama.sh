...
if ! curl -sS -f --max-time 30 -H "accept: application/json" "$BASE/models" >/dev/null; then
  ...
  echo "" >&2
  echo "From repo root:  docker compose up -d ollama" >&2
  echo "Then:            docker compose up ollama_pull" >&2
  exit 1
fi
echo "  OK"
...
RESP=$(curl -sS -f --max-time 120 -X POST "$BASE/chat/completions" \
  ...
  -d "$BODY_JSON") || {
  echo "FAIL: chat completion (model pulled? try: docker compose up ollama_pull or ollama pull $MODEL)" >&2
  exit 1
}
...
echo ""
echo "slackeel: Ollama smoke PASSED"
