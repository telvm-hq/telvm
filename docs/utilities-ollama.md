...
**Script paths:** see `**[scripts/README.md](../scripts/README.md)`** (`ollama/`, `closed-agent/`, …).
...
**Slackeel (standalone clone):** the `[slackeel/docs/OLLAMA.md](../slackeel/docs/OLLAMA.md)` copy is **self-contained** for developers who only clone Slackeel—keep pins and smoke behavior aligned when changing this file.
...
```bash
./scripts/ollama/smoke-ollama.sh
```
...
(`ollama_pull` is one-shot; first run downloads weights and can take several minutes.) Or bring up the full stack: `docker compose up --build`. Then rerun `**./scripts/ollama/smoke-ollama.ps1**` or `**./scripts/ollama/smoke-ollama.sh**`.
