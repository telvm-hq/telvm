...
Write-Host "telvm: Ollama utility smoke"
...
try {
  ...
  Write-Host ""
  Write-Host "Nothing is listening (connection refused or unreachable). From repo root, start Ollama first:" -ForegroundColor Yellow
  Write-Host "  docker compose up -d ollama" -ForegroundColor Gray
  Write-Host "  docker compose up ollama_pull   # one-shot: pull tiny models (first run can take minutes)" -ForegroundColor Gray
  Write-Host "Or full stack:  docker compose up --build" -ForegroundColor Gray
  Write-Host ""
  Write-Host "Then retry this script. If Ollama runs elsewhere, set OLLAMA_SMOKE_BASE (must end with /v1)." -ForegroundColor Yellow
  exit 1
}
Write-Host "  OK"
...
try {
  ...
  Write-Host "FAIL: chat completion request failed (model pulled? try: docker compose run --rm ollama_pull or ollama pull $model)" -ForegroundColor Red
  throw
}
...
Write-Host ""
Write-Host "telvm: Ollama utility smoke PASSED"
