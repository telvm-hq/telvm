...
unless config_env() == :test do
  ...
  if File.exists?(sock) do
    config :companion, :docker_adapter, Companion.Docker.HTTP
  end
end

if config_env() in [:dev, :test] do
  endpoint = Application.get_env(:companion, CompanionWeb.Endpoint, [])
  http = endpoint[:http] || []

  http =
    case System.get_env("PORT") do
      nil -> http
      p -> Keyword.put(http, :port, String.to_integer(p))
    end

  config :companion, CompanionWeb.Endpoint, http: http
end
...
if config_env() == :prod do
  ...
  config :companion, :dns_cluster_query, System.get_env("DNS_CLUSTER_QUERY")
